...
void main()
{
    ...
    if(horizontal)
    {
        for(int i = 1; i < 5; ++i)
        {
            result += texture(inputTexture, TexCoords + vec2(tex_offset.x * i, 0.0)).rgb * weight[i] * intensity;
            result += texture(inputTexture, TexCoords - vec2(tex_offset.x * i, 0.0)).rgb * weight[i] * intensity;
        }
    }
    else
    {
        for(int i = 1; i < 5; ++i)
        {
            result += texture(inputTexture, TexCoords + vec2(0.0, tex_offset.y * i)).rgb * weight[i] * intensity;
            result += texture(inputTexture, TexCoords - vec2(0.0, tex_offset.y * i)).rgb * weight[i] * intensity;
        }
    }
